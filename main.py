...
class ProxyProcessor(BaseHTTPRequestHandler):
    ...
    def correct_errors(url, content):
        ...
        if in_url('/company/'):
            content = re.sub(r'</li>(\s*)</div>(\s*)</ul>', r'</li>\g<1></ul>\g<2></div>', content)
    ...
    def change_direct_link(self, link):
        ...
        if port and port in self.direct_link:
            ...
            link = re.sub(r'https?://%s:%s' % (PROXY_HOST, port), '%s://%s' % (BASE_PROTO, host), link)
    ...
    def change_reverse_link(self, link):
        ...
        if host:
            ...
            if port:
                link = re.sub(r'https?://%s' % host, r'http://%s:%s' % (PROXY_HOST, port), link)
    ...
    def replace_content(self, response):
        if 'text/' in response.headers['Content-Type']:
            ...
            if 'window.location.href' in content:
                url = re.search(r'window.location.href\s?=\s?\'(.+?)\'', content)
                
                if url:
                    content = content.replace(url.group(1), self.change_reverse_link(url.group(1)))
    ...
    def send_headers(self, response):
        ...
        if 'Set-Cookie' in response.headers:
            splited_cookie = re.split(r'(?<!%s), ' % '|'.join(self.weekdayname), response.headers['Set-Cookie'])
    ...
    def send_error_page(self):
        ...
        self.wfile.write('<h1 align="center">500 Internal Server Error</h1>'.encode())
    ...
    def do_command(self, command):
        ...
        try:
            if command == 'do_GET':
                response = requests.get('%s://%s%s' % (BASE_PROTO, self.host, self.path),
                                        headers=headers,
                ...
                response = requests.post('%s://%s%s' % (BASE_PROTO, self.host, self.path),
                                         headers=headers,
                                         files=files,
                                         data=data,
                                         allow_redirects=False
